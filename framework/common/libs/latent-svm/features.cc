...
mxArray *process(const mxArray *mximage, const mxArray *mxsbin) {
  ...
  sbin = (int)mxGetScalar(mxsbin);

  // memory for caching orientation histograms & their norms
  blocks[0] = (int)round((double)dims[0]/(double)sbin);
  blocks[1] = (int)round((double)dims[1]/(double)sbin);
  hist = (double *)mxCalloc(blocks[0]*blocks[1]*18, sizeof(double));
  norm = (double *)mxCalloc(blocks[0]*blocks[1], sizeof(double));
  ...
  for (x = 0; x < out[1]; x++) {
    for (y = 0; y < out[0]; y++) {
      ...
      for (o = 0; o < 18; o++) {
		double h1 = fmin(*src * n1, 0.2);
		double h2 = fmin(*src * n2, 0.2);
		double h3 = fmin(*src * n3, 0.2);
		double h4 = fmin(*src * n4, 0.2);
		*dst = 0.5 * (h1 + h2 + h3 + h4);
		t1 += h1;
		t2 += h2;
		t3 += h3;
      ...
      for (o = 0; o < 9; o++) {
        ...
        double h1 = fmin(sum * n1, 0.2);
        double h2 = fmin(sum * n2, 0.2);
        double h3 = fmin(sum * n3, 0.2);
        double h4 = fmin(sum * n4, 0.2);
        *dst = 0.5 * (h1 + h2 + h3 + h4);
        dst += out[0]*out[1];
        src += blocks[0]*blocks[1];
      }
