function [hog,samples_x,samples_y]=poselet_compute_hog(img)
global config;
global ts;
...
assert(W>=config.HOG_CELL_DIMS(1) && H>=config.HOG_CELL_DIMS(2));
...
for x=1:config.HOG_CELL_DIMS(1)
   for y=1:config.HOG_CELL_DIMS(2)
       ...
       sub_mag = grad_mag(samples_y+y,samples_x+x);
       sub_ori = ori_hist(samples_y+y,samples_x+x,:);
               
       weight = (repmat(sub_mag,[1 1 num_angles]).*sub_ori)*w;
...
hog=reshape(hog,[num_cells(2:-1:1),num_hog_dims]);
