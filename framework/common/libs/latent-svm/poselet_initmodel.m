function model = poselet_initmodel(cls, pos, note, symmetry, sbin, sz)
...
% get an empty model
model = poselet_model_create(cls, note);
model.interval = 10;
...
% add root filter
global config;
[model, symbol, filter] = model_addfilter(model, zeros([sz config.NUM_POSELETS]), symmetry);
