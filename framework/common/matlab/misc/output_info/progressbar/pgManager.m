function handle = pgManager( handle,increment,string,titlestr )
%
% pgManager - shows a progress bar dialog based on the function "waitbar"
%
% Format: handle = pgManager( handle,increment [,string,titlestr] )
%
...
%           h           = pgManager( [],0,'my test' );
%           max_count   = 1e+3;
%           for idx = 1:max_count
%               fprintf( '%d\n',idx )';
%               h = pgManager( h,1/max_count );
%               if ~gui_active
%                   break;
%               end
%           end
...
%
    ...
    elseif (handle == last_handle)  % update last dialog
        counter_list(last_idx)  = increment;
...
%
    ...
    button_handle   = uicontrol( 'Parent',fig_handle,'units','pixels',...
        ...
        'Callback','gui_active(0);pgManager(get(gcbo,''parent''),-1);close(get(gcbo,''parent''));',...
        'CData',Icon16x16 );
...
%
    ...
    if (length(handle_list)>1)   
        y_increment = 100;                   % pixels
        x_increment = 60;                    % pixels
