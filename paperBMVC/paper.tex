...
\newcommand{\subsecnspc}{\vspace*{-1.4mm}}
...
For these annotations we followed the Pascal VOC guidelines. \green{In particular, we labeled each person with a bounding box which is the smallest rectangle containing its visible pixels. The bounding boxes are labelled as 'Truncated' if more than 15\%-20\% of the person is occluded or lies outside the bounding box. We also added a field  ``action" to each bounding box to describe which action is being executed.}% (note that there can be multiple actions for a single person).}
...
%Method  				  	 & mAP	& Accuracy  \\ \hline 
%Gupta~\etal~~\cite{Gupta09}	 & --	 & $78.67$ \\ \hline 
%BOF Image (B) 		 	 & 91.30 	& $85.00 $ \\ \hline 
%LSVM 			  		 & **	 & $** $ \\ \hline 
%LSVM + BOF Image (B)		& **	 & $** $ \\ \hline 
%\end{tabular}
%\caption{\cfs Comparison with the method of Gupta~\etal.~\cite{Gupta09} on their dataset. \normalsize}
%\label{tab:gupta}
%\end{table}
