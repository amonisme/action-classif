...
%                   cmpfile, objfile, cachedir, logtag);
    cmd = sprintf('%s %.6f %.6f %s %s %s %s %s %s %s %s %s', fullfile('./', LIB_DIR, 'latent-svm', 'learn'), ...
                    C, J, hdrfile, datfile, modfile, inffile, lobfile, ...
                    cmpfile, objfile, cachedir, logtag);
...
for i = 1:batchsize:numpos
  ...
  parfor k = 1:thisbatchsize
    ...
    pyra = my_featpyramid(imHOG, imBOF, model);
    [det, bs, info] = my_gdetect(pyra, model, 0, bbox, overlap);
    data{k}.bs = bs;
    data{k}.pyra = pyra;
    data{k}.info = info;
    if ~isempty(bs)
  ...
  for k = 1:thisbatchsize
    ...
    j = i+k-1;
    bs = my_gdetectwrite(data{k}.pyra, model, data{k}.bs, data{k}.info, 1, fid, j);
    if ~isempty(bs)
...
for i = 1:batchsize:numneg
  ...
  parfor k = 1:thisbatchsize
    ...
    pyra = my_featpyramid(color(imreadx(neg(j))), struct('size', [info.Height info.Width], 'feat', neg(j).feat), model);
    [dets, bs, info] = my_gdetect(pyra, model, -1.002);
    data{k}.bs = bs;
    data{k}.pyra = pyra;
    data{k}.info = info;
  end
  ...
  for k = 1:thisbatchsize
    j = inds(i+k-1);
    bs = my_gdetectwrite(data{k}.pyra, model, data{k}.bs, data{k}.info, ...
