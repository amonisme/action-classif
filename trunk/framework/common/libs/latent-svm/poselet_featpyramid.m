function pyra = featpyramid(img, model, padx, pady)
...
for i = 1:interval
  scaled = resize(img, 1/sc^(i-1));
  % "first" 2x interval
  pyra.feat{i} = poselet_features(model, sbin/2);
  pyra.scales(i) = 2/sc^(i-1);
  % "second" 2x interval
  pyra.feat{i+interval} = poselet_features(scaled, sbin);
  pyra.scales(i+interval) = 1/sc^(i-1);
  % remaining interals
  for j = i+interval:interval:max_scale
    scaled = resize(scaled, 0.5);
    pyra.feat{j+interval} = poselet_features(scaled, sbin);
