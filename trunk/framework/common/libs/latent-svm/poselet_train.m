function model = poselet_train(name, model, pos, neg, warp, randneg, iter, ...
...
for i = 1:numpos
  ...
  % get example
  im = warped{i};
  feat = poselet_features(im, model.sbin);
...
for i = 1:batchsize:numpos
  ...
  parfor k = 1:thisbatchsize
    ...
    pyra = poselet_featpyramid(im, model);
...
for i = 1:numneg
  ...
  im = imreadx(neg(i));
  feat = poselet_features(double(im), model.sbin);  
